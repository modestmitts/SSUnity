...
namespace PowerGridInventory
{
    ...
    [AddComponentMenu("Power Grid Inventory/PGI Slotable Item", 14)]
    [Serializable]
    public class PGISlotItem : MonoBehaviour, IEventSystemHandler, IComparable
    {
        ...
        /// <summary>
        /// The initial angle that this 3D icon you appear within an inventory slot.
        /// </summary>
        [Tooltip("The initial angle that this 3D icon will appear within an inventory slot.")]
        [HideInInspector]
        ...
        /// <summary>
        /// The number of items sharing the same <see cref="PGISlotItem.StackID"/> that can be stacked on top of each other in a single location.
        /// </summary>
        ...
        { 
            ...
            {
                _StackCount = value;
                if (_StackCount < 1) _StackCount = 1;
        ...
        public bool Rotate(RotateDirection dir)
        {
            ...
            //regarldess of where it is store, the item needs some basic
        ...
        public void ProcessStorage(PGIModel storage, int x, int y)
        {
            ...
            yInvPos = y;
            Model = storage;
            if (storage.ModifyTransforms)
            {
                if (this.transform.parent != storage.transform) PreviousParent = this.transform.parent;
                this.transform.parent = storage.gameObject.transform;
            }
        }
