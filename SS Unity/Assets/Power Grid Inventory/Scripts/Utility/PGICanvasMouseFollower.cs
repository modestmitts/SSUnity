...
namespace PowerGridInventory.Utility
{
    ...
    public class PGICanvasMouseFollower : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            transform.position = GetPointerPosOnCanvas(Canvas, PGIPointer.GetPosition());
        }
        ...
        public static Vector3 GetPointerPosOnCanvas(Canvas canvas, Vector2 pointerPos)
        {
            ...
            {
                return PGIPointer.GetPosition();

            }
            else
            {
                Vector3 globalMousePos;
                if (RectTransformUtility.ScreenPointToWorldPointInRectangle(canvas.transform as RectTransform, PGIPointer.GetPosition(), canvas.worldCamera, out globalMousePos))
                {
